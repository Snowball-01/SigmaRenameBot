...
async def startsequence_cmd(client, message):
    if message.from_user.id in SEQUENCE:
        return await message.reply_text("<b>Yᴏᴜ ᴀʀᴇ ᴄᴜʀʀᴇɴᴛʟʏ ɪɴ ᴀ ꜰɪʟᴇ sᴇQᴜᴇɴᴄɪɴɢ ᴘʀᴏᴄᴇss. Usᴇ /endsequence ᴄᴏᴍᴍᴀɴᴅ ᴛᴏ ꜰɪɴɪsʜ ɪᴛ.</b>")
    SEQUENCE[message.from_user.id] = True
    await message.reply_text("ʏᴏᴜ'ᴠᴇ sᴛᴀʀᴛᴇᴅ ᴀ ꜰɪʟᴇ sᴇQᴜᴇɴᴄɪɴɢ ᴘʀᴏᴄᴇss. Sᴇɴᴅ ᴛʜᴇ ꜰɪʟᴇs ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ sᴇQᴜᴇɴᴄᴇ ᴏɴᴇ ʙʏ ᴏɴᴇ.\n\nWʜᴇɴ ʏᴏᴜ'ʀᴇ ᴅᴏɴᴇ, Usᴇ /endsequence ᴛᴏ ꜰɪɴɪsʜ ᴀɴᴅ ɢᴇᴛ ᴛʜᴇ sᴇQᴜᴇɴᴄᴇᴅ ꜰɪʟᴇs.")

@Client.on_message(filters.private & filters.video & filters.create(notSEQUENCE))
async def sequencefiles_vid(client, message):
    
    media = message.video
    info = {'file_id': media.file_id, 'file_name': media.file_name}
    info1 = {media.file_name}
    try:
...
async def endsequence_cmd(client: Client, message):
    if message.from_user.id not in SEQUENCE:
        return await message.reply("You have not started a sequence yet. Use /startsequence command to start a new sequence.")
    if len(SEQUENCE_FILES) == 0:
        return await message.reply("No files to sequence. Send some files with /startsequence first.")
    sequence_files_names = sorted(SEQUENCE_FILES[message.from_user.id], key=lambda x: (type(x) is dict and x.get('file_name')) or x)
    
    try:
        channel = await client.ask(message.chat.id, "** (FORWRAD MESSAGE)\n\n Forward me message the channel you want to send these sequenced files to (make sure I'm admin in that channel).**\n\nIf you want files here send /no", timeout=60)
        ...
        if channel.text == '/no':
            ...
            for file_name in sequence_files_names:
                try:
                    vp = file_name
                    msg = await message.reply_cached_media(file_id=vp['file_id'], caption=vp['file_name'])
                except errors.FloodWait as e:
                    await asyncio.sleep(e.value)
                    await message.reply_cached_media(file_id=vp['file_id'], caption=vp['file_name'])
                    continue
            ...
            for file_name in sequence_files_names:
                try:
                    vp = file_name
                    await client.send_cached_media(chat_id=int(channel.forward_from_chat.id), file_id=vp['file_id'], caption=vp['file_name'])
                except errors.FloodWait as e:
                    await asyncio.sleep(e.value)
                    await client.send_cached_media(chat_id=int(channel.forward_from_chat.id), file_id=vp['file_id'], caption=vp['file_name'])
                    continue
            ...
            return await message.reply("**Invalid channel. Send /startsequence again. **")
    ...
    await message.reply(f"File sequencing completed. You have received or sent to target channel {len(sequence_files_names)} sequenced files.")
